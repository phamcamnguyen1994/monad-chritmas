...
const YAW_SENSITIVITY = 0.001 // Tốc độ xoay camera ngang (giảm độ nhạy)
const PITCH_SENSITIVITY = 0.001 // Tốc độ xoay camera dọc (giảm độ nhạy)
...
export function SledInputProvider({ children }) {
  ...
  const rotateBy = useCallback(
    (deltaYaw, deltaPitch) => {
      ...
      // Mouse phải: deltaYaw > 0 → quay sang phải (yaw tăng dương)
  ...
  const applyPointerDelta = useCallback(
    (dx, dy, source = 'pointer') => {
      if (!dx && !dy) return
      // Cho phép mouse movement khi pointer lock active
      if (source === 'mouse' && !pointerLockActive) return
      // Map trực tiếp: kéo chuột sang phải → yaw tăng dương (quay sang phải)
      rotateBy(-dx * YAW_SENSITIVITY, dy * PITCH_SENSITIVITY)
    },
    [rotateBy, pointerLockActive]
  )
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (event) => {
      // Xử lý mouse movement khi pointer lock active
      // movementX và movementY chỉ có khi pointer lock active
      const dx = event.movementX || 0
      const dy = event.movementY || 0
      if (dx !== 0 || dy !== 0) {
        applyPointerDelta(dx, dy, 'mouse')
      }
    }
