...
export default function MiniMap() {
  ...
  const { discovered, placements, player, deliveryMission } = useQuestStore((state) => ({
    discovered: state.discoveredDapps,
    placements: state.dappPlacements,
    player: state.playerPosition,
    deliveryMission: state.deliveryMission,
  }))
    ...
    return dapps
      .map((dapp) => {
        ...
        return {
          ...
          isTarget: deliveryMission.active && deliveryMission.targetId === dapp.id,
          left: toPercent(placement.x),
          top: 100 - toPercent(placement.z),
        }
      })
      .filter(Boolean)
  }, [placements, discovered, dapps, deliveryMission])
  ...
  return (
    <div className="minimap-root glass-panel">
      ...
      <div className="minimap-grid">
        ...
        {visibleDapps.map((dapp) => (
          <span
            key={dapp.id}
            className={`minimap-dot ${dapp.collected ? 'minimap-dot--collected' : ''} ${dapp.isTarget ? 'minimap-dot--target' : ''}`}
            style={{
              left: `${dapp.left}%`,
              top: `${dapp.top}%`,
            }}
